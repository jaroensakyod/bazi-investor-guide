...
async function main2() {
  const { getAllStocks } = await import("@/lib/investor/stock-database");
  ...
  console.log("\n════════ verdict คลังหุ้นทั้งหมด (ไทย " + stocks.filter((s) => s.country === "TH").length + " + โลก " + stocks.filter((s) => s.country !== "TH").length + ") ════════");
  console.log("\n-- เหมาะมาก (score ≥ 4) --");
  for (const r of scored.filter((r) => r.verdict === "very-good")) console.log(`  ✅✅ ${r.ticker} (${r.primaryElement}) score=${r.score}`);
  console.log("\n-- เหมาะ (score 2-3) --");
  for (const r of scored.filter((r) => r.verdict === "good").slice(0, 15)) console.log(`  ✅ ${r.ticker} (${r.primaryElement}) score=${r.score}`);
  console.log("\n-- กลาง (score 1) — ตัวอย่าง --");
  for (const r of scored.filter((r) => r.verdict === "neutral").slice(0, 10)) console.log(`  🟡 ${r.ticker} (${r.primaryElement}) score=${r.score}`);
  console.log("\n-- ควรเลี่ยง (score ≤ 0) — ตัวอย่าง --");
  for (const r of scored.filter((r) => r.verdict === "avoid").slice(0, 10)) console.log(`  ⛔ ${r.ticker} (${r.primaryElement}) score=${r.score}`);
}
