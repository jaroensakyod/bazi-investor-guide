/**
 * EXPAND universe ชุด 3: ปากีสถาน/ซาอุดีอาระเบีย/บราซิล (18 → 21 ตลาด)
 *
 * TradingView + is_primary=true:
 *   pakistan/PSX 60 · ksa/TADAWUL 60 · brazil/BMFBOVESPA 80
 * Yahoo suffix: KSE=.PK · TADAWUL=.SR · BOVESPA=.SA (เพิ่มใน yahoo.ts แล้ว)
 */
...
const MARKETS: Array<{ tv: string; exchange: string; country: string; market: string; currency: string; limit: number; name: string }> = [
  { tv: "pakistan", exchange: "PSX", country: "PK", market: "KSE", currency: "PKR", limit: 60, name: "ปากีสถาน" },
  { tv: "ksa", exchange: "TADAWUL", country: "SA", market: "TADAWUL", currency: "SAR", limit: 60, name: "ซาอุดีอาระเบีย" },
  { tv: "brazil", exchange: "BMFBOVESPA", country: "BR", market: "BOVESPA", currency: "BRL", limit: 80, name: "บราซิล" },
];
...
async function main() {
  ...
  if (added > 0) {
    ...
    db.meta.label = `หุ้นโลก (Wikipedia/TradingView — เอเชีย+ยุโรป+PK/SA/BR)`;
